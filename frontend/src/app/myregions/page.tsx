  ...
  useEffect(() => {
    const fetchSRMs = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/regions?managerId=${storedData}`); // Update this to your SRM-fetching endpoint
    ...
    <Layout>
      <div className="p-6">
        ...
        <table className="min-w-full mt-4 border border-gray-300">
          ...
          <tbody>
          {srms.map((srm) => {
            const manager = managers.find(mgr => mgr._id === srm.managerId); // Find the manager by ID
            return (
            <tr key={srm._id} className="border-b">
                ...
                {manager ? manager.username : 'No Manager Assigned'}
                </td>
        ...
        {isModalOpen && (
          <ModalSrms
            ...
            onSave={handleSave}
            initialData={selectedSRM || { nom: '', code: '', adresse: '', email: '', telph: '', status: 'inactive' }}
            isEdit={isEditMode}
            users={managers}
          />
        )}
